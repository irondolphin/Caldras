...
try:
    ...
    print("⚠️  WeasyPrint non disponibile. Funzionalità PDF disabilitate.")
from colorama import Fore, Style, init
...
def esporta_pdf(notes):
    if not WEASYPRINT_AVAILABLE:
        print("⚠️ Funzionalità PDF disabilitate. WeasyPrint non è disponibile.")
        return
    elenca_note(notes)
    try:
        ...
        if pw:
            ...
            access_granted()
            contenuto = decrypt_text(contenuto, pw)
        contenuto_html = markdown.markdown(contenuto)
        html_body = f"<h1>{titolo}</h1>{contenuto_html}<hr><p style='font-size:10pt;color:#999;'>🛰️ Sigillo Caldras: C-LDRS.{datetime.datetime.now().strftime('%m%d')}.∞</p>"
        html = f"<html><head><style>body{{font-family:sans-serif;padding:2em;}}</style></head><body>{html_body}</body></html>"
        filename = f"{titolo.replace(' ', '_')}.pdf"
        HTML(string=html).write_pdf(filename)
        print(Fore.GREEN + f"📤 PDF '{filename}' esportato con successo.")
    except:
        print("⚠️ Errore nell'esportazione.")

def esporta_pdf_alternativa(notes):
    elenca_note(notes)
    try:
        i = int(input("Numero della nota da esportare: ")) - 1
        titolo, contenuto, *resto = notes[i]
        ...
        html_body = f"<h1>{titolo}</h1>{contenuto_html}<hr><p style='font-size:10pt;color:#999;'>🛰️ Sigillo Caldras: C-LDRS.{datetime.datetime.now().strftime('%m%d')}.∞</p>"
        html = f"<html><head><meta charset='utf-8'><style>body{{font-family:sans-serif;padding:2em;}}</style></head><body>{html_body}</body></html>"

        with tempfile.NamedTemporaryFile('w', delete=False, suffix=".html", encoding='utf-8') as fhtml:
            fhtml.write(html)
            html_path = fhtml.name

        pdf_filename = f"{titolo.replace(' ', '_')}.pdf"
        result = subprocess.run(["wkhtmltopdf", html_path, pdf_filename], capture_output=True)

        if result.returncode == 0:
            print(Fore.GREEN + f"📤 PDF '{pdf_filename}' esportato con successo.")
        else:
            print(Fore.RED + "⚠️ Errore durante la conversione PDF.")
            print(result.stderr.decode())

    except Exception as e:
        print(Fore.RED + f"⚠️ Errore nell'esportazione: {type(e).__name__} — {e}")
...
def menu():
    ...
    while True:
        ...
        print("  9. Esporta PDF (alternativa con wkhtmltopdf)")
        print("  0. Esci")
        print("╚══════════════════════════════════════════════╝")
        scelta = input(">>> ").strip()
